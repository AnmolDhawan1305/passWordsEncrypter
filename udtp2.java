...
import java.util.*;
import java.io.*;
class udtp2 extends JFrame implements FocusListener,ActionListener,ItemListener{
	...
	udtp2(String s1,String s2,String s3){
		setSize(450,700);
		...
		j1.setBounds(100,600,100,50);
		j2.setBounds(250,600,100,50);
		add(l1);
		add(l2);
		add(l3);
		add(p);
	   ...
	   try{
		...
		while(true){
			...
			if(s[0].equals(t1.getText().toLowerCase()) && s[1].equals(t3.getText().toLowerCase()) && s[2].equals(t2.getText())){
				...
				int a[]=new int[p.getText().length()];
				a=dx.encode(p.getText()); 
	...
	public void itemStateChanged(ItemEvent e){
		...
		if(!(c.getSelectedItem().equals("Other"))){
			this.hide();
			new udtp(t1.getText(),t2.getText(),c.getSelectedItem(),p.getText()).show();
		}
	}
	}
